...
for version in v416 v417 v418 v419 v420; do
    pipeline_file="$PROJECT_ROOT/.tekton/forklift-fbc-comp-stage-${version}-pull-request.yaml"
    if [ -f "$pipeline_file" ]; then
        if grep -q "contains.*MTV-VERSION" "$pipeline_file"; then
            echo "  ✅ .tekton/forklift-fbc-comp-stage-${version}-pull-request.yaml"
        else
            echo "  ❌ .tekton/forklift-fbc-comp-stage-${version}-pull-request.yaml (not updated)"
        fi
    else
        echo "  ❓ .tekton/forklift-fbc-comp-stage-${version}-pull-request.yaml (file not found)"
    fi
done
echo
